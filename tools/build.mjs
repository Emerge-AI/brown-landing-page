...
function srcset(img, ext) {
  return img.widths.map((w) => `img/${img.slug}-${w}.${ext} ${w}w`).join(', ');
}
function picture(img, { sizes, eager = false, className = '' } = {}) {
  ...
  <img src="img/${img.slug}-${mid}.jpg" srcset="${srcset(img, 'jpg')}" sizes="${sizes}"
...
/* ---------- JSON-LD ---------- */
function jsonLd() {
  const imgUrl = (slug, w) => `${SITE.origin}${SITE.path}img/${slug}-${w}.jpg`;
  const graph = {
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'General Dentistry', item: SITE.origin + '/general-dentistry' },
          { '@type': 'ListItem', position: 3, name: 'Dental Implants', item: CANONICAL },
        ],
      },
    ],
  };
...
const navDesktop = NAV.map((n, i) => n.items
  ? `<li>
      ...
      <ul class="mhb-header__dropdown" id="dd-${i}">
        ${n.items.map(([t, h]) => `<li><a href="${h}"${h === '/dental-implants/' ? ' aria-current="page"' : ''}>${esc(t)}</a></li>`).join('\n        ')}
      </ul>
    </li>`
  : `<li><a class="mhb-header__nav-link" href="${n.href}">${esc(n.label)}</a></li>`).join('\n    ');

const navDrawer = NAV.map((n, i) => n.items
  ? `<li>
      ...
      <ul class="mhb-drawer__sub" id="dsub-${i}">
        ${n.items.map(([t, h]) => `<li><a href="${h}">${esc(t)}</a></li>`).join('\n        ')}
      </ul>
    </li>`
  : `<li><a class="mhb-drawer__link" href="${n.href}">${ic(n.icon)} ${esc(n.drawerLabel || n.label)}</a></li>`).join('\n    ');
    ...
    <div class="mhb-footer__grid">
      ...
      <div>
        ...
        <ul>
          ...
          <li><a href="#what-is-an-implant">What is a dental implant?</a></li>
          <li><a href="#cost">Cost &amp; financing</a></li>
          <li><a href="#faq">Implant FAQs</a></li>
        </ul>
      </div>
    </div>
...
/* Inline all CSS to avoid the @import request waterfall (7 serial requests
   otherwise). Order matters: tokens first, then components. */
const CSS_FILES = [
  'tokens/colors.css', 'tokens/typography.css', 'tokens/spacing.css',
  'tokens/radii-shadows.css', 'tokens/base.css', 'landing.css', 'chrome.css',
];
...
<head>
  ...
  <title>${esc(SITE.metaTitle)}</title>
  <meta name="description" content="${esc(SITE.metaDesc)}">
  ...
  <meta property="og:title" content="${esc(SITE.metaTitle)}">
  <meta property="og:description" content="${esc(SITE.metaDesc)}">
  <meta property="og:url" content="${CANONICAL}">
  <meta property="og:image" content="${SITE.origin}${SITE.path}img/${IMAGES.hero.slug}-1200.jpg">
  <meta property="og:image:width" content="1200">
  <meta property="og:image:height" content="800">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="${esc(SITE.metaTitle)}">
  <meta name="twitter:description" content="${esc(SITE.metaDesc)}">
  <meta name="twitter:image" content="${SITE.origin}${SITE.path}img/${IMAGES.hero.slug}-1200.jpg">

  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Inter:ital,wght@0,300..800;1,400..600&family=Playfair+Display:ital,wght@0,600..800;1,600..700&display=swap" media="print" onload="this.media='all'">
  <noscript><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Inter:ital,wght@0,300..800;1,400..600&family=Playfair+Display:ital,wght@0,600..800;1,600..700&display=swap"></noscript>
  ${heroPreload}
  <style>
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>${CANONICAL}</loc>
    <lastmod>${process.env.BUILD_DATE ?? new Date().toISOString().slice(0, 10)}</lastmod>
...
/* Netlify redirects. The bare domain sends visitors to the landing page, and
   the secondary domain 301s to the canonical one so the two don't split
   search authority. www → apex is handled by Netlify's primary-domain setting. */
const SECONDARY = 'marshallhbrown.com';
const REDIRECTS = `# Bare domain → the landing page
/                                    ${SITE.path}                          301

# Secondary domain → canonical domain (path preserved)
...
fs.writeFileSync(path.join(PAGE, 'favicon.svg'), FAVICON);
// sitemap/robots/redirects/headers must sit at the domain root
...
console.log(`built docs${SITE.path}index.html (${(html.length / 1024).toFixed(1)} KB)`);
console.log('wrote favicon.svg, sitemap.xml, robots.txt, _redirects, _headers');
